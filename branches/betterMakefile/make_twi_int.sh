...
do
  ...
  MCU=$name OUTDIR=../avr-drv/lib/$name TARGET=twiInt make -s -C TWI clean
  MCU=$name OUTDIR=../avr-drv/lib/$name TARGET=twiInt make -s -k -C TWI
  code=$?
  if (( code )); then
...
MCU=at90can128 OUTDIR=../avr-drv/lib/at90can128 TARGET=twiInt make -s -C TWI clean
